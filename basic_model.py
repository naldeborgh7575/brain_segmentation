...
class BasicModel(object):
    def __init__(self, single_or_cascade='single', n_epoch=10):
    ...
    def compile_model(self):
        ...
        single.add(Flatten())
        single.add(Dense(256))
        single.add(Dropout(0.5))
        single.add(Dense(5))
        single.add(Activation('softmax'))

        sgd = SGD(lr=0.01, decay=0.1)
    ...
    def fit_model(self, X_train, y_train):
        ...
        self.model_comp.fit(X_train, Y_train, batch_size=32, nb_epoch=self.n_epoch, validation_split=0.1, show_accuracy=True, verbose=1)
