...
class KerasModel():
    def __init__(self, h_1=65, w_1=65, h_2=33, w_2=33, n_filters = [64, 64, 160, 5], n_classes = 5, n_chan = 4, n_epoch = 3, batch_size = 5, pool_size_1 = 4, pool_size_2 = 2, local_conv_1 = 7, local_conv_2 = 3, global_conv = 13, output_conv = 21, learning_rate = 0.005, decay_factor = 0.1, momentum_coef_1 = 0.5, momentum_coef_2 = 0.9):
        self.h_1 = h_1
        self.h_2 = h_2
        self.w_1 = w_1
        self.w_2 = w_2
    ...
    def compile_model(self):
        ...
        model.add_node(Convolution2D(nb_filter=self.n_filters[0], nb_row=self.local_conv_1, nb_col=self.local_conv_1, border_mode='valid', activation='relu', input_shape=(4,65,65), W_regularizer=l1l2(l1=0.01, l2=0.01), W_constraint=maxnorm(2)), name='local_k11', input='input_dropout')
        ...
        model.add_node(Convolution2D(nb_filter = self.n_filters[1], nb_row=self.local_conv_2, nb_col=self.local_conv_2, activation='relu', border_mode='valid', W_regularizer=l1l2(l1=0.01, l2=0.01), W_constraint=maxnorm(2)), name='local_k21', input='drop_p11')
        ...
        model.add_node(Convolution2D(nb_filter=self.n_filters[2], nb_row=self.global_conv, nb_col=self.global_conv, border_mode='valid', activation='relu', W_regularizer=l1l2(l1=0.01, l2=0.01), W_constraint=maxnorm(2)), name='global_1', input='drop_g1')
        ...
        model.add_node(Convolution2D(nb_filter=self.n_filters[3], nb_row=self.output_conv, nb_col=self.output_conv, border_mode='valid', activation='relu', W_regularizer=l1l2(l1=0.01, l2=0.01), W_constraint=maxnorm(2)), name='merge_1', inputs=['drop_lp21', 'drop_global'], merge_mode='concat', concat_axis=1)
        ...
        model.add_node(Convolution2D(nb_filter=self.n_filters[0], nb_row=self.local_conv_1, nb_col=self.local_conv_1, border_mode='valid', activation='relu', W_regularizer=l1l2(l1=0.01, l2=0.01), W_constraint=maxnorm(2)), name='local_k12', inputs=['merge_drop', 'input2_drop'], merge_mode='concat', concat_axis=1)
        ...
        model.add_node(Convolution2D(nb_filter=self.n_filters[1], nb_row=self.local_conv_2, nb_col=self.local_conv_2, border_mode='valid', activation='relu', W_regularizer=l1l2(l1=0.01, l2=0.01), W_constraint=maxnorm(2)), name='local_k22', input='drop_lp12')
        ...
        model.add_node(Convolution2D(nb_filter=self.n_filters[2], nb_row=self.global_conv, nb_col=self.global_conv, border_mode='valid', activation='relu', W_regularizer=l1l2(l1=0.01, l2=0.01), W_constraint=maxnorm(2)), name='global_2', inputs=['merge_drop', 'input2_drop'], merge_mode='concat', concat_axis=1)

        model.add_node(Dropout(0.75), name='drop_lp22', input='local_p22')
        model.add_node(Dropout(0.75), name='drop_global22', input='global_2')
        model.add_node(Convolution2D(nb_filter=self.n_filters[3], nb_row=self.output_conv, nb_col=self.output_conv, border_mode='valid', activation='relu', W_regularizer=l1l2(l1=0.01, l2=0.01), W_constraint=maxnorm(2)), name='merge_2', inputs=['drop_lp22', 'drop_global22'], merge_mode='concat', concat_axis=1)
        ...
        sgd = SGD(lr=0.005, decay=0.1, momentum=0.5, nesterov=True)
    ...
    def fit_model(self, X_train, X_33_train, y_train):
        ...
        self.model_comp.fit(data, batch_size=self.batch_size, nb_epoch = self.n_epoch, show_accuracy = True, verbose=1, validation_split=0.1)
